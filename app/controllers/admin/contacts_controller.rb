class Admin::ContactsController < ApplicationController
  ...
  before_action :load_all_stores, only: [:new, :edit]
  ...
  def create
    ...
    respond_to do |format|
      if @contact.save
        ...
        format.json { render :show, status: :created, location: [:admin, @contact] }
      else
  ...
  def update
    respond_to do |format|
      if @contact.update(contact_params)
        ...
        format.json { render :show, status: :ok, location: [:admin, @contact] }
      else
  ...
  def contact_params
    params.require(:contact).permit(:name, :email, :phone, :address, :store_id)
  end
end
